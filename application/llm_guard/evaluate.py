...
def scan_output(
    scanners: list[OutputScanner], prompt: str, output: Union[str, List[str]], fail_fast: bool = False, only_json: bool = False
) -> tuple[Union[str, List[str]], dict[str, bool], dict[str, float]]:
    """
    ...
    if len(scanners) == 0 or output.strip() == "" if type(output) == str else len(output) == 0:
        return sanitized_output, results_valid, results_score

    start_time = time.time()
    for scanner in scanners:
        start_time_scanner = time.time()
        if type(scanner).__name__ == "JSON":
            sanitized_output, is_valid, risk_score = scanner.scan(prompt, sanitized_output, only_json) # type: ignore
        else:
            sanitized_output, is_valid, risk_score = scanner.scan(prompt, sanitized_output) # type: ignore
        elapsed_time_scanner = time.time() - start_time_scanner
