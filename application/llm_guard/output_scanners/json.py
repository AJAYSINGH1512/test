...
class JSON(Scanner):
    ...
    def scan(self, prompt: str, output: str, only_json: bool = False) -> tuple[Union[str, List[str]], bool, float]:
        ...
        if len(valid_jsons) < self._required_elements:
            print("#"*100)
            print("not all required json elements are Found")
            LOGGER.warning(
        ...
        print("#"*100)
        if only_json:
            final_sanitized_output = valid_jsons
        else:
            final_sanitized_output = output
        return final_sanitized_output, True, -1.0
